...
| `src/natsnim/conn.nim` | `nats.go` (`Conn`: connect, publish, subscribe, request, flush, reconnect) | **transport rewritten, not translated.** Go's reader goroutine + channels + mutexes are replaced by a single-threaded poll(2)-driven socket. Deviations: no background thread, so reconnect is **lazy** (driven by the waiting calls, one attempt per `reconnectWaitMs` window) where Go reconnects in a goroutine; `unsubscribe` discards pending (Go's `Unsubscribe`); pending-limit overrun is raised by the next `nextMsg` instead of going to an async error callback; one server URL, so no server-pool iteration; no jitter, TLS, nkeys/JWT or WebSocket. Resubscription keeps each subscription's original sid, as upstream does. |
