...
| Nim | upstream | notes |
|---|---|---|
| `src/natsnim/parser.nim` | `parser.go` | state machine, `MsgArgs`, `parseInt64`. Deviations (documented in the file): no `scratch`/`argBuf`/`msgBuf` reuse, so no `cloneMsgArg`; the payload is always copied; no statistics (they belong to the connection). State enum keeps Go's iota order. |
| `src/natsnim/nuid.nim` | `nuid` `nuid.go` @ v1.0.1 | per-instance sequential PRNG instead of Go's globally seeded `math/rand`; the process-global generator is **not** mutex-guarded (no threads by design); `newNuID(seed)` is exposed for tests. |
| `src/natsnim/subject.nim` | `nats.go` (`badSubject`, `badQueue`) | no client-side wildcard matching exists to port: the server routes by sid. |
...
| `src/natsnim/conn.nim` | `nats.go` (`Conn`: connect, publish, subscribe, request, flush, reconnect) | **transport rewritten, not translated.** Go's reader goroutine + channels + mutexes are replaced by a single-threaded, nonblocking state machine (`poll(2)`, one absolute deadline per public call; reconnect phases survive across zero-timeout polls). Deviations: no background thread, so reconnect is **lazy** (driven by the waiting calls, paced by a persisted schedule) where Go reconnects in a goroutine; `unsubscribe(maxMsgs)` is honored server-side with remaining-count re-ARM on reconnect (Go drops the local queue); a pending-limit overrun is raised by the next `nextMsg` instead of going to an async error callback; one server URL resolved once at dial (reconnect reuses the address; no DNS inside reconnect); no server-pool iteration, TLS, nkeys/JWT or WebSocket. Resubscription keeps each subscription's original sid, as upstream does. Bounded writes (pointer/length, EINTR-safe) and `TCP_NODELAY`; parser/control-line/payload/queue limits enforce memory bounds. |
| `tests/t_bus.nim` (no-responders) | `nats.go` (`noResponders` handling) | a 503 status is surfaced as an error rather than a message; asserted to fail fast, and that the connection stays usable |
| `tests/t_reconnect_opts.nim` | – | the reconnect knobs as pure functions (`dialTimeoutForAttempt`, `reconnectDelayMs`): a black-hole dial and a retry storm cannot be produced reliably in a test, but the arithmetic that decides them can. |
| `tests/t_reconnect.nim` | – | outage properties upstream cannot test from one process: it restarts a real server on the same port. Asserts transparent reconnection with sids preserved, in-order delivery of buffered publishes, the buffer cap failing loud, `reconnect: false` / `maxReconnects: 0` being fatal (and publishes failing fast instead of buffering), `close` not being an outage, and the attempt-spacing window (no spin, no storm). |
| `src/natsnim.nim` | `gokr/natswrapper` over `nats.c` (its public shape) | the shim: identical names/signatures to the binding Niffler uses, so adoption is a `requires` change. Adds `natsConnection_Publish` (data + length, binary-safe) and `natsMsg_GetHeader`, which `natswrapper` does not surface. Handles are GC-managed, so `*Destroy` detaches rather than frees. |
